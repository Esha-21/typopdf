...
class rnn(TFBaseModel):
    ...
    def sample(self, cell):
        ...
        initial_input = tf.concat([
            ...
            tf.zeros(tf.stack([self.num_samples, 2]), dtype=tf.float32),
            tf.ones(tf.stack([self.num_samples, 1]), dtype=tf.float32),
    ...
    def calculate_loss(self):
        ...
        # self.bias = tf.placeholder_with_default(tf.zeros_like(self.num_samples, dtype=tf.float32))
        self.bias = tf.zeros_like(self.num_samples, dtype=tf.float32)

            # tf.zeros([self.num_samples], dtype=tf.float32), [None])
            # tf.zeros(tf.stack([self.num_samples]), dtype=tf.float32))
