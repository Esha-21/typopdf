...
class LSTMAttentionCell(tf.nn.rnn_cell.RNNCell):
    ...
    def zero_state(self, batch_size, dtype):
     return LSTMAttentionCellState(
        tf.zeros(tf.stack([batch_size, self.lstm_size]), dtype=dtype),
        tf.zeros(tf.stack([batch_size, self.lstm_size]), dtype=dtype),
        tf.zeros(tf.stack([batch_size, self.lstm_size]), dtype=dtype),
        tf.zeros(tf.stack([batch_size, self.lstm_size]), dtype=dtype),
        tf.zeros(tf.stack([batch_size, self.lstm_size]), dtype=dtype),
        tf.zeros(tf.stack([batch_size, self.lstm_size]), dtype=dtype),
        tf.zeros(tf.stack([batch_size, self.num_attn_mixture_components]), dtype=dtype),
        tf.zeros(tf.stack([batch_size, self.num_attn_mixture_components]), dtype=dtype),
        tf.zeros(tf.stack([batch_size, self.num_attn_mixture_components]), dtype=dtype),
        tf.zeros(tf.stack([batch_size, self.window_size]), dtype=dtype),
        tf.zeros(tf.stack([batch_size, self.char_len]), dtype=dtype),
    )
    ...
    def _parse_parameters(self, gmm_params, eps=1e-8, sigma_eps=1e-4):
        ...
        pis = pis * (1 + tf.expand_dims(self.bias, 0))
        # sigmas = sigmas - tf.expand_dims(self.bias, 1)
        sigmas = sigmas - tf.expand_dims(self.bias, 0)
